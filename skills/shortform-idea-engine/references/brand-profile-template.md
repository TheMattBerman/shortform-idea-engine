...
> **Canonical source:** The schema in this file is owned by the `brand-profiler` skill (`skills/brand-profiler/references/brand-profile-template.md`). This copy must stay in sync with that file. Any schema change must be applied to both files at the same time.
